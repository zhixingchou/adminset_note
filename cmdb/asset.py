...
def asset(request):
    ...
    # get方法，只能获得数组的最后一个元素，必须使用getlist才能获取整个数组 by zhouzx
    ...
    if status:
        # status包含xx，若忽略大小写使用__icontains. by zhouzx
        asset_find = asset_find.filter(status__contains=status)
    if keyword:
        # Q 对象 (django.db.models.Q) 对象用于封装一组关键字参数，Q 对象可以使用& 和| 操作符 --by zhouzx
