...
def get_dir(args):
    # Python的ConfigParser Module中定义了3个类对INI文件进行操作。分别是RawConfigParser、ConfigParser、SafeConfigParser。RawCnfigParser是最基础的INI文件读取类，
    # ConfigParser、SafeConfigParser支持对%(value)s变量的解析。
    # by zhouzx
    config = ConfigParser.RawConfigParser()
    dirs = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # os.path.abspath = os.path.dirname + os.path.basename
    # 使用 with 语句操作文件对象. by zhouzx
    with open(dirs+'/adminset.conf', 'r') as cfgfile:
        # readfp(fp) 可以读取一打开的文件. by zhouzx
        config.readfp(cfgfile)
        a_path = config.get('config', 'ansible_path')
        r_path = config.get('config', 'roles_path')
        p_path = config.get('config', 'playbook_path')
        ...
        mongodb_pwd = config.get('mongodb', 'mongodb_pwd')
        mongodb_collection = config.get('mongodb', 'collection')
        # -get(section,option) 得到section中option的值，返回为string类型. by zhouzx
        webssh_domain = config.get('webssh', 'domain')
    # 根据传入参数返回变量以获取配置，返回变量名与参数名相同
    if args:
        # vars() 函数返回对象object的属性和属性值的字典对象。 by zhouzx
        return vars()[args]
    else:
