...
def write_role_vars(roles, vargs):
    ...
    for r in roles:
        ...
        if vargs:
            ...
            with open(roles_dir+r+'/vars/main.yml', 'wb+') as role_file:    # wb+ 以二进制读写模式打开
...
def playbook(request):
    ...
    if request.method == 'POST':
        ...
        if host:
            if roles:
                if role_vars:
                    write_role_vars(roles, role_vars)   # 执行roles，自定义vars会在centos中生成roles/xx/vars/main.yml文件，写入vars配置
                for h in host:
                    logging.info("==========ansible tasks start==========")
                    logging.info("User:"+request.user.username)
                    logging.info("host:"+h)
                    with open(ansible_dir + '/gexec.yml', 'w+') as f:   # w+ 以读写模式打开
                        flist = ['- hosts: '+h+'\n', '  remote_user: root\n', '  gather_facts: true\n', '  roles:\n']
                        for r in roles:
                            rs = '    - ' + r + '\n'
                            flist.append(rs)
                ...
                for h in host:
                    for p in pbook:
                        f = open(playbook_dir + p, 'r+')    # r+ 打开一个文件用于读写。文件指针将会放在文件的开头。
                        flist = f.readlines()
                        flist[0] = '- hosts: '+h+'\n'
                        f = open(playbook_dir + p, 'w+')    # 	打开一个文件用于读写。如果该文件已存在则将其覆盖。如果该文件不存在，创建新文件。
                        f.writelines(flist)
                        f.close()
...
def host_sync(request):
    ...
    ansible_file = open(ansible_dir+"/hosts", "wb")     # wb  以二进制写模式打开
    all_host = Host.objects.all()
